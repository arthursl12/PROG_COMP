...
void filter_size(vector<vector<int>>& valids, int target_size, vector<vector<int>>& filtered){
    filtered.clear();
    for(int i=0; i<valids.size(); i++){
        if(valids[i].size() == target_size){
            filtered.push_back(valids[i]);
        }
    }
}

// Acha todas as combinações de somas de {1,2,3} para atingir o objetivo
void combinationalSum(vector<int> numbers, int target_sum, 
                      vector<vector<int>>& valids, 
                      vector<int>& current_stack, int idx)
{
    if (target_sum == 0){
        valids.push_back(current_stack);
        return;
    }

    while(idx < numbers.size() && target_sum - numbers[idx] >= 0){
        current_stack.push_back(numbers[idx]);
        combinationalSum(numbers, target_sum-numbers[idx], valids, current_stack, idx);
        idx++;
...
void get_permutations(vector<vector<int>> v, set<vector<int>>& res){
    res.clear();
    for (int i=0; i<v.size(); i++){
        vector<int> v_copy = v[i];
        do{
            res.insert(v_copy);
...
void filter_horizontal_sum(string s, set<vector<int>> all_permutations, 
...
void process_horizontal(string s, int target_size,
    ...
    filter_size(valids, target_size, filtered_valids);
    ...
    get_permutations(filtered_valids, all_permutations);
    // for(auto it=all_permutations.begin(); it!=all_permutations.end(); it++){
    //     print_vector(*it);
    // }
    ...
    // print_vector(result);
    // vector<vector<int>> filtered_valids_sum;
    filter_horizontal_sum(s, all_permutations, filtered_valids_sum);
